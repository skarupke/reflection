...
struct JsonWriter
{
	...
	template<typename It>
	It string_to_json(Range<const char> str, It out)
	{
		*out++ = '"';
		for (char c : str)
		{
	...
	It enum_to_json(const MetaReference & object, It out)
	{
		...
		return string_to_json(Range<const char>(info->GetAsHashedString(object).get()), out);
	}
	...
	struct MemberPrinter
	{
		...
		void operator()(It & lhs, const T & rhs) const
		{
			*lhs++ = ',';
			...
			lhs = copy_string(" : ", lhs);
			switch(rhs.GetType().access_type)
			{
            case PASS_BY_VALUE:
				lhs = writer.to_json(rhs.Get(object).GetVariant(), lhs);
				return;
			case PASS_BY_REFERENCE:
				lhs = writer.to_json(rhs.GetRef(const_cast<MetaReference &>(object)), lhs);
				return;
            case GET_BY_REF_SET_BY_VALUE:
            {
                lhs = writer.to_json(const_cast<MetaReference &>(static_cast<const MetaReference &>(rhs.GetCRef(object))), lhs);
				return;
			}
			}
...
struct JsonParser
{
	...
	ParseResult<void> list_from_json(MetaReference & object, ParseState state)
	{
		...
		return parse_json_list(state, [&](ParseState state) -> ParseResult<void>
		{
			...
			return from_json(reference, state) >>= [&](ParseSuccess<void> success) -> ParseResult<void>
			{
	...
	ParseResult<void> map_from_json(MetaReference & object, ParseState state)
	{
		...
		MetaOwningMemory key({ key_buffer.get(), key_buffer.get() + info->key_type.GetSize() }, info->key_type);
		...
		{
			MetaReference reference = key.GetVariant();
			return from_json(reference, state) >>= [&](ParseSuccess<void> success) -> ParseResult<MetaReference>
			{
				return ParseSuccess<MetaReference>(std::move(reference), success.new_state);
			};
		},
		[&](ParseState state, ParseSuccess<MetaReference> key_success) -> ParseResult<void>
		{
			MetaOwningMemory value({ value_buffer.get(), value_buffer.get() + info->mapped_type.GetSize() }, info->mapped_type);
			MetaReference reference = value.GetVariant();
			return from_json(reference, state) >>= [&](ParseSuccess<void> success) -> ParseResult<void>
			{
				info->insert(object, std::move(key_success.result), std::move(reference));
				return std::move(success);
			};
		});
	}
	...
	ParseResult<void> parse_member(ParseState state, MetaReference & object, const MemberType & member)
	{
        ...
        case PASS_BY_VALUE:
		{
			...
			return from_json(as_reference, state) >>= [&](ParseSuccess<void> success) -> ParseResult<void>
			{
        ...
        RAW_THROW(std::runtime_error("missing case in the switch statement above"));
	}
	template<typename MemberType>
    ...
    ParseResult<ReusableStorage<ClassHeaderList>::Reusable> parse_struct_begin(ParseState state)
	{
		...
		{
			return parse_json_map_content(state, &parse_identifier, [&](ParseState state, ParseSuccess<Range<const char> > identifier_success)
			{
...
bool JsonSerializer::deserialize(MetaReference to_fill, Range<const char> json_text) const
{
